...
export const submitClub = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (existingClub) {
			throw new Error(
				`The club "${args.clubName}" has already been submitted.`,
			);
		}
	...
	handler: async (ctx, args) => {
		...
		for (const club of args.clubs) {
			try {
				...
				const existingClub = await ctx.db
					.query("clubSubmissions")
					.withIndex("by_club_name", (q) =>
						q.eq("clubName", club.clubName.trim()),
					)
					.first();

				if (existingClub) {
					errors.push(
						`The club "${club.clubName}" has already been submitted.`,
					);
					continue;
				}
