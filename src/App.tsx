...
export default function App() {
	...
	const parsedClubNames = useMemo(
		() => clubs.map((c) => c.clubName.trim()).filter(Boolean),
		[clubs],
	);
	...
	const validateClubName = (value: string, index: number) => {
		...
		const dup = clubs.some(
			(c, i) => i !== index && c.clubName.trim().toLowerCase() === lower,
		);
		if (dup) return "Duplicate";
		// Already submitted
		if ((allClubs || []).some((c) => c.clubName.trim().toLowerCase() === lower))
			return "Exists";
		return "";
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		newErrors.submitterName = validateField(
			"submitterName",
			formData.submitterName,
		);
		newErrors.submitterEmail = validateField(
			"submitterEmail",
			formData.submitterEmail,
		);
		newErrors.submitterSchoolEmail = validateField(
			"submitterSchoolEmail",
			formData.submitterSchoolEmail,
		);
		newErrors.clubs = clubs.map((c, i) => validateClubName(c.clubName, i));
		setErrors(newErrors);

		const hasErrors = !!(
			newErrors.submitterName ||
			newErrors.submitterEmail ||
			newErrors.submitterSchoolEmail ||
			newErrors.clubs.some((e) => e)
		);
		if (hasErrors) {
		...
		const validClubs = clubs
			.filter((c, i) => !newErrors.clubs[i] && c.clubName.trim())
			.map((c) => ({ clubName: c.clubName.trim(), description: "" }));
		...
		try {
			...
			if (result.success) {
				toast.success(
					`Successfully submitted ${result.totalSubmitted} club${result.totalSubmitted !== 1 ? "s" : ""}!`,
				);
	...
	useEffect(() => {
		setErrors((prev) => ({
			...prev,
			clubs: clubs.map((c, i) => validateClubName(c.clubName, i)),
		}));
	}, [clubs, allClubs]);
	...
	return (
		<div className="min-h-screen flex flex-col" id="top">
			...
			<header
				className="relative overflow-hidden pb-10 pt-16 animate-fade-in"
				role="banner"
			>
					...
					<div className="text-center">
						...
						<div className="max-w-3xl mx-auto card-glass card-glow-border p-8 md:p-10 animate-rise">
							<div className="flex items-center justify-center mb-6">
								<div
									className="w-16 h-16 rounded-2xl bg-gradient-to-br from-grayson-gold via-grayson-green to-grayson-navy shadow-brand-soft flex items-center justify-center"
									aria-hidden="true"
								>
									<svg
										className="w-8 h-8 text-white"
										role="img"
										aria-label="Menu layers icon"
										fill="currentColor"
										viewBox="0 0 20 20"
									>
							...
							<p className="text-base md:text-lg text-grayson-navy/85 leading-relaxed">
								Know awesome clubs at Grayson High School? Submit them below to
								be featured on the bingo card.
							</p>
							<div className="mt-8 grid sm:grid-cols-2 gap-4">
								<div className="rounded-xl bg-grayson-navy-soft/40 border border-white/30 px-5 py-4 backdrop-blur-sm flex flex-col items-center text-center">
									...
									<p className="text-sm font-bold text-grayson-navy">
										Total Clubs Submitted
									</p>
									<p className="text-3xl font-extrabold gradient-text-gold mt-1 tabular-nums">
										{clubsCount}
									</p>
								</div>
								<div className="rounded-xl bg-grayson-green-soft/40 border border-white/30 px-5 py-4 backdrop-blur-sm flex flex-col items-center text-center">
									...
									<p className="text-sm mt-1 text-grayson-navy/90">
										Participants will complete their bingo card and claim
										exclusive prizes, given out at the NHS booth.
									</p>
								</div>
							</div>
						</div>
			...
			<main id="main-content" className="flex-1">
				...
				<section
					...
					aria-labelledby="submit-clubs-heading"
				>
					...
					<div className="relative max-w-3xl mx-auto">
						<div className="card-glass card-glow-border p-8 md:p-10 animate-rise">
							<h2
								id="submit-clubs-heading"
								className="text-2xl md:text-3xl font-bold text-grayson-navy text-center mb-8"
							>
								Submit Club(s)
							</h2>
							<form onSubmit={handleSubmit} className="space-y-10">
								...
								<div className="space-y-6" aria-labelledby="your-info-heading">
									<div className="flex items-center gap-3">
										<h3
											...
											className="text-lg font-semibold text-grayson-navy tracking-wide"
										>
											Your Information
										</h3>
											...
											>
												<Input
													...
													onBlur={(e) =>
														setErrors((prev) => ({
															...prev,
															submitterSchoolEmail: validateField(
																"submitterSchoolEmail",
																e.target.value,
															),
														}))
													...
													aria-describedby={
														errors.submitterSchoolEmail
															? "error-submitterSchoolEmail"
															: "school-email-domain"
													}
													required
													error={!!errors.submitterSchoolEmail}
												...
												<span
													...
													className="px-3 py-2 text-sm select-none flex items-center text-grayson-navy/70 border-l border-border-subtle/60 bg-white/50"
												>
													@g.gcpsk12.org
												</span>
											</div>
								...
								<div className="space-y-6" aria-labelledby="club-info-heading">
													...
													>
														<Input
															...
															onChange={(e) => {
																...
																setClubs((prev) =>
																	prev.map((c, i) =>
																		i === index ? { ...c, clubName: value } : c,
																	),
																);
																setErrors((prev) => ({
																	...prev,
																	clubs: prev.clubs.map((err, i) =>
																		i === index
																			? validateClubName(value, index)
																			: err,
																	),
																}));
															}}
															onBlur={(e) =>
																setErrors((prev) => ({
																	...
																	clubs: prev.clubs.map((err, i) =>
																		i === index
																			? validateClubName(e.target.value, index)
																			: err,
																	),
																}))
															}
									...
									<div className="flex flex-wrap items-center gap-3 text-[10px] text-grayson-navy/60 pt-1">
										<span className="px-2 py-0.5 rounded bg-grayson-gold-soft/60 text-grayson-navy/80 font-medium">
											{parsedClubNames.length} name
											{parsedClubNames.length !== 1 && "s"}
										</span>
								...
								<div>
									<button
										...
										disabled={
											...
											!!errors.submitterEmail ||
											!!errors.submitterSchoolEmail ||
											errors.clubs.some((e) => e) ||
											!parsedClubNames.length
										}
										...
										aria-label={
											isSubmitting
												? "Submitting clubs"
												: `Submit ${parsedClubNames.length} clubs`
										}
									>
										{isSubmitting ? (
				...
				<section
					...
					aria-labelledby="all-clubs-heading"
				>
					...
					<div className="relative max-w-6xl mx-auto">
						...
						{allClubs.length === 0 ? (
							<div className="card-glass p-12 text-center">
								<p className="text-grayson-navy/70 text-lg">
									No clubs have been submitted yet. Be the first!
								</p>
							</div>
						) : (
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
								{allClubs.map((club) => (
									...
									>
										...
										<h3
											id={`club-title-${club._id}`}
											className="text-base font-semibold text-grayson-navy mb-2 tracking-tight"
										>
											{club.clubName}
										</h3>
										<div className="flex items-center justify-between text-[10px] text-grayson-navy/60 pt-1 mt-2 border-t border-white/40">
											<p>
												By{" "}
												<span className="font-medium text-grayson-navy/80">
													{club.submitterName}
												</span>
											</p>
											<p className="tabular-nums">
												{new Date(club._creationTime).toLocaleDateString()}{" "}
												{new Date(club._creationTime).toLocaleTimeString([], {
													hour: "2-digit",
													minute: "2-digit",
												})}
											</p>
			...
			{/* Footer */}
			<footer
				className="mt-auto relative bg-grayson-navy text-white/90 py-10"
				role="contentinfo"
			>
